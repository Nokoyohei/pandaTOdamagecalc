  ...
  <v-container>
    ...
    <v-row>
      ...
      <v-col cols="12" md="7" order-md="0">
        <BasePowerSlider v-model="localBasePower" :default-power="SKILL_POWER.ElectroAttack" />
        <StatsTextField
...
const localBasePower = ref(SKILL_POWER.ElectroAttack)

const idealDamage = computed(() =>
  magicAttackPower(SkillPower.ElectroAttack(localBasePower.value), buffedMA.value, 25)
)
...
const resMA = computed(() => {
  const needMA = calcNeedStats(
    ...
    calcMonsterDef(monster.value, 'magic'),
    monster.value.elecR,
    SkillPower.ElectroAttack(localBasePower.value) / 100,
    buffedMA.value,
    25
  )
