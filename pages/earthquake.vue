  ...
  <v-container>
    ...
    <v-row>
      ...
      <v-col cols="12" md="7" order-md="0">
        <BasePowerSlider v-model="localBasePower" :default-power="SKILL_POWER.Earthquake" />
        <StatsTextField
...
const localBasePower = ref(SKILL_POWER.Earthquake)

const idealDamage = computed(() =>
  magicAttackPower(SkillPower.Earthquake(localBasePower.value), buffedMA.value, 25)
)
...
const resMA = computed(() => {
  const needMA = calcNeedStats(
    ...
    calcMonsterDef(debuffedMonster.value, 'magic'),
    debuffedMonster.value.earthR,
    SkillPower.Earthquake(localBasePower.value) / 100,
    buffedMA.value,
    25
  )
