...
//   読む順序 : viewer/public/<name>.json（git 管理で update-data が毎回更新する。ESAction_* は SkillESAction.json に統合済み）
//              → json_output/<name>.json（ローカルで変換した全テーブル、git 管理外。変換した時点の古いデータになりがち）
//   CI では viewer/public だけを sparse checkout して使う。ローカルも同じ入力になる
...
export function loadTable(bot, name) {
  if (name.startsWith('ESAction_')) {
    ...
    if (existsSync(mergedPath)) {
      if (!mergedCache.has(mergedPath)) mergedCache.set(mergedPath, JSON.parse(readFileSync(mergedPath, 'utf8')))
  ...
  throw new Error(`${name}: not found under ${bot} (viewer/public or json_output)`)
}
